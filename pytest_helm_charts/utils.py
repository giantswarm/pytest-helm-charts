...
def wait_for_objects_condition(
    ...
    Args:
        ...
        obj_type: type of the objects to check; they're usually derived from
        ...
        objs_namespace: namespace where all the resources are created, if they are Namespace-scoped
            (single namespace for all resources)
        is_global: if True, the object is expected to be Cluster-scoped, if False then Namespace-scoped and the
            objs_namespace above must be a valid namespace.
...
def wait_for_namespaced_objects_condition(
    ...
    return wait_for_objects_condition(
        kube_client, obj_type, obj_names, objs_namespace, False, obj_condition_func, timeout_sec, missing_ok
    )
...
def wait_for_global_objects_condition(
    ...
    Args:
        kube_client: client to use to connect to the k8s cluster
        obj_type: type of the objects to check; they're mostly derived from
            [APIObject](pykube.objects.APIObject)
        obj_names: a list of object resource names to check; all of the objects must be pass `obj_condition_fun`
            for this function to succeed
        timeout_sec: timeout for the call
